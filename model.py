...
class UnaryOperatorNode(OperatorNode):
    def __init__(self, valueNode, sym):
        super().__init__(sym)
        self.valueNode = valueNode

    def to_string(self):
        return "(" + self.symbol + " " + self.valueNode.to_string() + ")"
    ...
    def __eq__(self, other):
        if self.__class__ != other.__class__: return False
        if self.symbol != other.symbol: return False
        if self.valueNode != other.valueNode: return False
        return True

class PlusPlusNode(UnaryOperatorNode):
    def __init__(self, valueNode):
        super(PlusPlusNode, self).__init__(valueNode, "++")

    def eval(self):
        return self.valueNode.eval() + 1
...
class MinusMinusNode(UnaryOperatorNode):
    def __init__(self, valueNode):
        super(MinusMinusNode, self).__init__(valueNode, "--")
